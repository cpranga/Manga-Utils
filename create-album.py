    ...
    if args.folder:
        for file in fullpaths:
                if os.path.isfile(file) == False or len(file.split(".")) != 2 or file.split(".")[1] == "zip":
                    continue
                if os.path.isdir(os.path.join(args.output, dir_name)) == False:
                    os.mkdir(os.path.join(args.output, dir_name))
                shutil.copy(file, os.path.join(args.output, dir_name, str(counter) + "."+ file.split(".")[1]))
                if args.reverse:
                    counter -= 1
                else:
                    counter += 1
